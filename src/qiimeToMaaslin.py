...
class CClade:
    ...
    # Recursively travel the length of a tree until you find the
    # terminal node (where astrClade == False or actually []) or a
    # dict key that matches the clade call.  If at any time a clade is
    # given that is not currently know, return a new clade which is
    # set to the current Clade as a child.
    ...
    # This allows you to recursively impute values for clades without
    # values given their children counts.  Assumably this should be
    # called only once and after all clade abundances have been added.
    # If the m_adValues already exist return the stored
    # m_adValues. (No imputation needed).  Otherwise call impute for
    # all children and take the sum of the values from all the
    # children by column Not a sum of a list but summing a list with
    # lists by element.
    def impute(self):
    ...
    # Update the given hashValues dict with clade abudances given
    # depth specifications Return a set of integers returning an
    # indicator of the structure of the tree preserved in the
    # dict/hash When the appropriate level of the tree is reached
    # Hashvalue is updated with the clade (including lineage) and the
    # abundance. looks like {"clade":blist(["0.0", "0.1"...])}
    def _freeze(self, hashValues, iTarget, astrClade, iDepth, fLeaves):
        # fHit is true on atleast one of the following conditions:
        # iTarget is not 0 indicating no changes Leaves are indicated
        # to be only given and the target depth for the leaves is
        # reached.  The required depth is reached.
        fHit = (not iTarget) or \
               ((fLeaves and (iDepth == iTarget))
                or ((not fLeaves) and (iDepth <= iTarget)))
        # Increment depth
        iDepth += 1
        # Returns a set
        setiRet = set()
        # If there are children build integer set indicating structure
        # of the tree preserved in the dict
        if self.m_hashChildren:
            ...
            for strChild, pChild in self.m_hashChildren.items():
                setiRet |= pChild._freeze(
                    hashValues, iTarget, astrClade + [strChild], iDepth,
                    fLeaves)
        ...
        # if astrClade is not == [] (so you are actually in a clade of
        # the tree) And the clade has values (should be true, if not
        # impute should have been callded before running this method)
        # And we are at the correct level of the tree then Add to the
        # dict the clade and the abundance values
        if astrClade and self.m_adValues and fHit:
            hashValues[
                c_cOutputOTULineageDelim.join(astrClade)
            ] = self.m_adValues
        return setiRet

    # Call helper function setting the clade and depth to defaults
    # (start positions) The important result of this method is
    # hashValues is updated with clade and abundance information
    ...
    def _repr(self, strClade):
        ...
        if strClade:
            ...
            if self.m_hashChildren:
                strRet += " "
        if self.m_hashChildren:
            strRet += " ".join(p._repr(s)
                               for (s, p) in self.m_hashChildren.items())
        return (strRet + ">")
...
def funcDetectFormat(strFormatInQuestion):
    ...
    # Given a string, a function is returned that will parse it or an
    # error is given.
    lFormats = [[r'^k__(\w)+;\w', qiimeFormat1],
                [r'^Root',        qiimeFormat2],
                [r'^k__(\w)+; ',  qiimeFormat3],
                [r'[^;]',         funcPass]]
...
def qiimeFormat1(strConsensusLineage):
    ...
    # Remove root
    strConsensusLineage = re.sub(
        r'^k__$', "k__"+c_strUnclassified, strConsensusLineage)
    # Change no end clade to unclassified
    strConsensusLineage = re.sub(
        r'__$', "__"+c_strUnclassified, strConsensusLineage)
    # Update Internal unclassified
    strConsensusLineage = re.sub(
        r'__;', "__"+c_strUnclassified+";", strConsensusLineage)
    # Remove root
    strConsensusLineage = re.sub(
        r';', c_cOutputOTULineageDelim, strConsensusLineage)
...
def qiimeFormat2(strConsensusLineage):
    ...
    strConsensusLineage = re.sub(
        r'^Root$', c_strUnclassified, strConsensusLineage)
    # Remove root
...
def funcPass(strConsensusLineage):
    """If no formatting is needed use this function (for instance if no
    clade level is given k__Bacteria)

    :param strConsensusLineage: The conensus lineage to format (or not
    format in this case)
    :type strConsensusLineage: String
...
def funcFormatInput(strmInputFile):
    """Formats the Qiime Table and returns the metadata (dict) and data
    (list of lists)
    ...
    for astrLine in strmInputFile:
        ...
        if astrLine[0][0] == c_strOTUID[0]:
            ...
            # Get otu and data
            strOTU, astrData, strConsensusLineage = (astrLine[0],
                                                     astrLine[1:-1],
                                                     astrLine[-1])
            ...
            lstrOutputlines.append(
                [c_strOutputDelimiter.join(
                    [c_cOutputOTULineageDelim.join(
                        [strConsensusLineage, strOTU])]
                )]+astrData)

    if fDetectFormatter:
        print("""qiimeToMaAsLin did not detect a specific format for
        your file, this could be due to using a new consensus lineage
        format, improper formatting, or having consensus lineages with
        no heirarchical relationship (if not using a heirarchical
        relationship, you may ignore this message). Please check your
        output file formatting and the read me for supported
        formats. If this is a standard format and needs to be
...
def merge_metadata(aastrMetadata, aastrData, ostm, fNormalize,
                   strMissing, astrExclude, dMin, iTaxa, fLeaves):
    """Joins and outputs a data matrix with a metadata matrix, optionally
    normalizing and filtering it.  A pipe-delimited taxonomy hierarchy
    can also be dynamically added or removed.
    ...
    :param    dMin:            Minimum fraction of maximum value for
                              per-column quality control.
    :type    dMin:            bool
    :param    iTaxa:           Depth of taxonomy to be computed, -1 = leaves
                               only, 0 = no change
    :type    iTaxa:            int
    :param    fLeaves:        Output only leaves, not complete taxonomy;
    ignored if taxa = 0
    :type    fLeaves:        bool

    Metadata are optional; if not provided, data will be optionally
    normalized or its taxonomy modified as requested.  Metadata are
    provided one row per sample, data one column per sample, both
    files tab-delimited text with one header row and one header
    column.

    Metadata IDs that do not match data IDs are discarded, and data
    IDs without corresponding metadata IDs are given missing values.
    Missing data values are always treated (and output) as zero.

    Per-column quality control is performed if the requested minimum
    fraction is greater than zero.  Specifically, for each column i,
    the row j containing the maximum value d is identified.  If d is
    less than the minimum fraction of row j's maximum value over all
    columns, the entire column i is removed.

    A taxonomy hierarchy will be calculated by default if row IDs are
    pipe-delimited, i.e. of the form A|B|C.  All parent clades are
    computed by default, e.g. A|B and A, save when they would be
    identical to a more specific child clade.  Negative values are
    counted from the bottom (right) of the hierarchy rather than the
    top.  The special value of 0 deactivates hierarchy calculation.

    >>> aastrMetadata = [[t.strip() for t in s]
                         for s in ("-YZ", "a1x", "b0y", "c z")]
    >>> aastrData = [s.split() for s in ( \
        "-    a    b    c",        \
        "A|B    1    2    3",    \
        "A|C    4    5    6",    \
        "D|E    7    8    9")]
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout,
                       True, "", [], 0.01, -1,
                       False) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    Z    x    y    z
    A    0.416667    0.466667    0.5
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout,
                       True, "", [], 0.01, -1,
                       True) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    Z    x    y    z
    A|B    0.0833333    0.133333    0.166667
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout, True,
                       "", [], 0, 0,
                       True) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout, True,
                       "", [], 0, 1, False) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    Z    x    y    z
    A    0.416667    0.466667    0.5
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout, True,
                       "", [], 0, -1, True) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    Z    x    y    z
    A|B    0.0833333    0.133333    0.166667
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout, False, "",
                       [], 0, 0, True) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    Y    1    0
    Z    x    y    z
    A|B    1    2    3
    ...
    >>> merge_metadata(aastrMetadata, aastrData, sys.stdout, True, "-",
                       [], 0.8, 0, True) # doctest: +NORMALIZE_WHITESPACE
    sample    b    c
    Y    0    -
    Z    y    z
    A|B    0.133333    0.166667
    ...
    >>> merge_metadata(None, aastrData, sys.stdout, False, "",
                       [], 0, 0, True) # doctest: +NORMALIZE_WHITESPACE
    sample    a    b    c
    A|B    1    2    3
    A|C    4    5    6
    ...
    # Put metadata in a dictionary {"First line element", ["line
    # element 2", "line element 3", "line element 4"]}
    astrMetadata = None
    hashMetadata = {}
    ...
    # Read in the data Find the header column (iCol)
    # assumed to be 1 or 2 depending on the location of
    # "NAME" Create a list (adSeq) that will eventually
    # hold the sum of the columns of data
    ...
    for astrLine in aastrData:
        if astrHeaders:
            ...
            try:
                ...
                # If a data row can not be parsed then
                # hold in list and Eventually print at
                # the end of the output file as is.
                # This being the case thier abundance
                # will not be added to the sample
                # Summaries and will not be used in
                # normalization.
            except ValueError:
            ...
            if (iCol > 1) and (astrLine[0] != astrLine[1]):
                if astrLine[1].find(astrLine[0]) >= 0:
                    ...
                    astrLine[0] += " " + astrLine[1]
            pTree.get(astrLine[0].split(c_cOutputOTULineageDelim)).set(
                adCounts)
            # There are 2 formats one can have depending on "NAME"
            # location Set the column header index
        else:
            iCol = 2 if (astrLine[1].upper() == "NAME") else 1
            astrHeaders = [strCur.replace(" ", "_")
                           for strCur in astrLine[iCol:]]
            adSeqs = [0] * len(astrHeaders)

    # Create tree of data Input missing data Fill
    # hashFeatures with the clade name (key) and a blist
    # of values (value) of the specified level interested.
    if iTaxa:
        pTree.impute()
    hashFeatures = {}
    ...
    if dMin > 0:
        ...
        for i in range(len(astrHeaders)):
            ...
            dMaxThem = max(
                aadData[iMax][j]
                for j in (range(i) + range(i + 1, len(astrHeaders)))
            )
    ...
    adMult = [((c_dTarget / d) if (fNormalize and (d > 0)) else 1)
              for d in adSeqs]
        ...
        for i in range(len(astrMetadata)):
            ...
            for strSubject in astrHeaders:
                ...
                if not astrSubject:
                    ...
                    astrSubject = hashMetadata.get(strSubject, [])
                astrCur.append(astrSubject[i] if (i < len(astrSubject))
                               else "")
    ...
    for iFeature in range(len(astrFeatures)):
        ...
        for i in range(len(astrValues)):
            ...
            elif not strValue or (
                    type(strValue) == str and len(strValue) == 0):
...
argp = argparse.ArgumentParser(
    prog="qiimeToMaaslin.py",
    description="""Join a qiime otu table with a metadata matrix, optionally
    normalizing and filtering it.A pipe-delimited taxonomy hierarchy can
    also be dynamically added or removed."""
)

argp.add_argument(
    "-n", dest="fNormalize", action="store_false",
    help="Don't normalize data values by column sums")
argp.add_argument(
    "-s", dest="strMissing", metavar="missing",
    type=str,                default=" ",
    help="String representing missing metadata values")
argp.add_argument(
    "-m", dest="dMin", metavar="min",
    type=float,        default=0.00,
    help="Per-column quality control, minimum fraction of maximum value")
argp.add_argument(
    "-t", dest="iTaxa", metavar="taxa",
    type=int, default=-1,
    help="Depth of taxonomy to be computed, negative=from right, 0=no change")
argp.add_argument(
    "-l", dest="fLeaves", action="store_true",
    help="Output only leaves, not complete taxonomy")
argp.add_argument(
    "-x", dest="istmExclude", metavar="exclude.txt", type=file,
    help="File from which sample IDs to exclude are read")
argp.add_argument(
    "istmMetadata", metavar="metadata.txt", type=file, nargs="?",
...
def _main():
    ...
    merge_metadata(args.istmMetadata and
                   csv.reader(args.istmMetadata, csv.excel_tab),
                   lstrExtractedData, sys.stdout, args.fNormalize,
                   args.strMissing, args.istmExclude, args.dMin,
                   args.iTaxa, args.fLeaves)
